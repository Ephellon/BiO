...
WebInspector.SASSSupport.parseSCSS = function (url, content) {
  ...
  return WebInspector.formatterWorkerPool.runTask("parseSCSS", {
    content: content
  }).then(onParsed);
...
WebInspector.SASSProcessor.processCSSEdits = function (astService, map, ranges, newTexts) {
  ...
  function onCSSParsed(newCSSAST) {
    ...
    var nodeMapping = new Map();
    var rebasedMap = (map.rebase(clonedModels, nodeMapping));
...
WebInspector.SASSProcessor.SetTextOperation.fromCSSChange = function (change, map) {
  var oldProperty = (change.oldProperty());
  var newProperty = (change.newProperty());
...
WebInspector.SASSProcessor.SetTextOperation.prototype = {
  ...
  rebase: function (newMap, nodeMapping) {
    var sassNode = (nodeMapping.get(this._sassNode)) || this._sassNode;
...
WebInspector.SASSProcessor.TogglePropertyOperation.fromCSSChange = function (change, map) {
  var oldCSSProperty = (change.oldProperty());
...
WebInspector.SASSProcessor.TogglePropertyOperation.prototype = {
  ...
  rebase: function (newMap, nodeMapping) {
    var sassProperty = (nodeMapping.get(this._sassProperty)) || this._sassProperty;
...
WebInspector.SASSProcessor.RemovePropertyOperation.fromCSSChange = function (change, map) {
  var removedProperty = (change.oldProperty());
...
WebInspector.SASSProcessor.InsertPropertiesOperation.fromCSSChange = function (change, map) {
  ...
  if (!sassRule)
    return null;
  var insertedProperty = (change.newProperty());
...
WebInspector.SASSProcessor.InsertPropertiesOperation.prototype = {
  ...
  rebase: function (newMap, nodeMapping) {
    var sassRule = (nodeMapping.get(this._sassRule)) || this._sassRule;
    var afterSASSProperty = this._afterSASSProperty ? (nodeMapping.get(this._afterSASSProperty)) || this._afterSASSProperty : null;
...
WebInspector.ASTSourceMap.prototype = {
  ...
  compiledModel: function () {
    return (this._models.get(this._compiledURL));
  },
  sourceModels: function () {
    var sourceModels = (new Map(this._models));
  ...
  rebase: function (updated, outNodeMapping) {
    ...
    for (var i = 0; i < compiledNodes.length; ++i) {
      ...
      var sourceNode = (this._compiledToSource.get(compiledNode));
      var mappedCompiledNode = (outNodeMapping.get(compiledNode) || compiledNode);
      var mappedSourceNode = (outNodeMapping.get(sourceNode) || sourceNode);
...
WebInspector.SASSSourceMapFactory.prototype = {
  editableSourceMap: function (target, sourceMap) {
    ...
    for (let url of sourceMap.sourceURLs()) {
      ...
      var sassPromise = contentProvider.requestContent().then(text => this._astService.parseSCSS(url, text || "")).then(ast => models.set(ast.document.url, ast));
      promises.push(sassPromise);
    }
    var cssURL = sourceMap.compiledURL();
    var cssPromise = header.originalContentProvider().requestContent().then(text => this._astService.parseCSS(cssURL, text || "")).then(ast => models.set(ast.document.url, ast));
    promises.push(cssPromise);
    return Promise.all(promises).then(this._onSourcesParsed.bind(this, sourceMap, models)).catchException((null));
  },
