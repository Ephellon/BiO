WebInspector.EditFileSystemView = function (fileSystemPath) {
  ...
  this._eventListeners = [WebInspector.fileSystemMapping.addEventListener(WebInspector.FileSystemMapping.Events.FileMappingAdded, this._update, this), WebInspector.fileSystemMapping.addEventListener(WebInspector.FileSystemMapping.Events.FileMappingRemoved, this._update, this), WebInspector.isolatedFileSystemManager.addEventListener(WebInspector.IsolatedFileSystemManager.Events.ExcludedFolderAdded, this._update, this), WebInspector.isolatedFileSystemManager.addEventListener(WebInspector.IsolatedFileSystemManager.Events.ExcludedFolderRemoved, this._update, this)];
...
WebInspector.EditFileSystemView.prototype = {
  ...
  renderItem: function (item, editable) {
    ...
    if (item instanceof WebInspector.FileSystemMapping.Entry) {
      ...
      var pathPrefix = (editable ? item : WebInspector.UIString("%s (via .devtools)", item));
      var pathPrefixElement = element.createChild("div", "file-system-value");
  ...
  commitEdit: function (item, editor, isNew) {
    ...
    if (item instanceof WebInspector.FileSystemMapping.Entry) {
      ...
      if (!isNew)
        WebInspector.isolatedFileSystemManager.fileSystem(this._fileSystemPath).removeExcludedFolder((item));
